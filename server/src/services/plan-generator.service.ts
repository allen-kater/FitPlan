...
import { lookupQuota, QuotaEntry } from './quota.service.js';
...
export interface PlanCalculationResult {
  ...
  restDayCarbG: number;
  /** 力训日碳水配额 g/kg */
  trainingDayCarbQuota: number;
  /** 休息日碳水配额 g/kg */
  restDayCarbQuota: number;
  /** 蛋白质配额 g/kg */
  proteinQuota: number;
  planType: string;
}
...
export function calculateCardioCalorie(
  ...
  if (!hasCardio || goal !== 'FAT_LOSS' || weight >= 80) return 0;
  if (!exerciseHR || !restingHR || !duration) return 0;
...
export function calculateMaintenanceCalories(
  tdee: number, trainingCalorie: number, cardioCalorie: number,
...
export function calculateTargetCalories(
  trainingDayMaintenance: number, restDayMaintenance: number, goal: string,
...
export function calculateMacronutrients(
  ...
  const quota = proteinQuota ?? lookupQuota(gender, goal, 170, weight).protein;
...
/**
 * Determine plan type code
 */
export function determinePlanType(goal: string, trainingTime: string, hasCardio: boolean): string {
  if (goal === 'FAT_LOSS') {
    const mapping: Record<string, string> = {
      EARLY_MORNING: hasCardio ? 'F-01' : 'F-02', LATE_MORNING: hasCardio ? 'F-03' : 'F-04',
      BEFORE_LUNCH: 'F-05', AFTER_LUNCH: 'F-06', BEFORE_DINNER: 'F-07', AFTER_DINNER: 'F-08', NIGHT: 'F-08',
    };
    return mapping[trainingTime] ?? 'F-01';
  }
  const mapping: Record<string, string> = {
    EARLY_MORNING: 'M-01', LATE_MORNING: 'M-02', BEFORE_LUNCH: 'M-03',
    AFTER_LUNCH: 'M-04', BEFORE_DINNER: 'M-05', AFTER_DINNER: 'M-06', NIGHT: 'M-07',
  };
  return mapping[trainingTime] ?? 'M-01';
}

/**
 * Complete plan generation calculation (8-step algorithm)
 */
export function generatePlanCalculation(data: BodyDataInput): PlanCalculationResult {
  ...
  const cardioCalorie = calculateCardioCalorie(
    data.cardioHeartRate, data.restingHeartRate, data.cardioDuration, data.weight, data.goal, data.hasCardio,
  );
  const { trainingDayMaintenance, restDayMaintenance } = calculateMaintenanceCalories(tdee, trainingCalorie, cardioCalorie);
  const { trainingDayTarget, restDayTarget } = calculateTargetCalories(trainingDayMaintenance, restDayMaintenance, data.goal);

  // 从准确配额表查找配额（根据性别、目标、身高、体重）
  const quota: QuotaEntry = lookupQuota(data.gender, data.goal, data.height, data.weight);

  const { proteinG, fatG, trainingDayCarbG, restDayCarbG } = calculateMacronutrients(
    trainingDayTarget, restDayTarget, data.weight, data.gender, data.goal, quota.protein,
  );
  ...
  return {
    ...
    trainingDayCarbQuota: quota.trainingDayCarb,
    restDayCarbQuota: quota.restDayCarb,
    proteinQuota: quota.protein,
    planType,
  };
}
